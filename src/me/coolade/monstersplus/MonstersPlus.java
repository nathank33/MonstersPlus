...
public class MonstersPlus extends JavaPlugin
{
    ...
    public static Location getRandomValidLocation(Location loc, int xRad, int yRad, int zRad)
    {
    	...
    	for(int i = 0; i < MAX_ATTEMPTS; i++)
    	{
    		...
    		Location tempLoc = new Location(loc.getWorld(),
    				loc.getX() + randx, loc.getY() + randy, loc.getZ() + randz);
    ...
    { 
        AttributeInstance attributes = ((EntityInsentient)((CraftLivingEntity)lent).getHandle()).getAttributeInstance(GenericAttributes.d);
    ...
    { 
        AttributeInstance attributes = ((EntityInsentient)((CraftLivingEntity)lent).getHandle()).getAttributeInstance(GenericAttributes.e);
        return attributes.getValue();
    }
    ...
    public static boolean isSpawnableLocation(Location loc)
    {
		...
		if(!worldName.equalsIgnoreCase("world"))// && !worldName.equalsIgnoreCase("skyworld") && !worldName.equalsIgnoreCase("world_the_nether")) 
			return false;
				...
				&& Bukkit.getPluginManager().getPlugin("mcore") != null)
		{
			...
			if(!faction.getFlag(FFlag.MONSTERS)) return false;
		}
    ...
    public static boolean isBuildLocation(Player player,Location loc)
    {
		...
		if(Bukkit.getPluginManager().getPlugin("Factions") != null
				&& Bukkit.getPluginManager().getPlugin("mcore") != null)
		{
			if(!FactionsListenerMain.canPlayerBuildAt(player, PS.valueOf(loc.getBlock()),true)) 
				return false;
		}
		return true;
    } 
