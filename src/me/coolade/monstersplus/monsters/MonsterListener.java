...
public class MonsterListener implements Listener
{
	...
	private static final int MOBSPAWNER_MAXMOBS = 45; //The maximum amount of monsters allowed in the radius 

	@EventHandler(priority = EventPriority.NORMAL)
	public void onCreatureSpawn(CreatureSpawnEvent event) 
	{ 
		if(!MonstersPlus.isSpawnableLocation(event)) return;
		else if(event.getSpawnReason().equals(SpawnReason.SPAWNER)) return;
		...
		if(event.getSpawnReason().equals(SpawnReason.SPAWNER_EGG))
		{
			 ...
			 * and if there are, we can allow the spawn type to include these unique monsters.
			 */
	...
	{
		/*	Check to see if the entities target is a player and then 
			if the entity has a custom name, show it as visible
	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event)
	{
		...
		if(damager instanceof Projectile){
			damager = (Entity)((LivingEntity)((Projectile) damager).getShooter());
			isProjectile = true;
		}
	...
	public void onEntityDeath(EntityDeathEvent event)
	{
		new MonsterUpdater(event);
		/**
		 * If the one of the items that is being dropped has a custom name then we want it to drop
		 * with full durability restored.
		 */
		Entity ent = event.getEntity();
		if(ent instanceof LivingEntity && !(ent instanceof Player))
		{
			...
			if(MonsterList.isCustomMonster(lent) && 
					(lent instanceof Zombie) || (lent instanceof Skeleton))
			{
	...
	/*
	 * Monster Cleaner, use this to prevent too many monsters from spawning from MobSpawners
	 */
	@EventHandler(priority = EventPriority.NORMAL)
