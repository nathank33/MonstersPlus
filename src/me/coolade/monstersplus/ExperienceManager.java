 ...
 * 
 *         Credit to nisovin
 *         (http://forums.bukkit.org/threads/experienceutils-make
 *         -giving-taking-exp-a-bit-more-intuitive.54450/#post-1067480) for an
 *         implementation that avoids the problems of getTotalExperience(),
 *         which doesn't work properly after a player has enchanted something.
 * 
 *         Credit to comphenix for further contributions: See
 *         http://forums.bukkit
 *         .org/threads/experiencemanager-was-experienceutils
 *         -make-giving-taking-
...
public class ExperienceManager {
	...
	private void setExp(double base, double amt) {
		...
		double pct = (base - getXpForLevel(newLvl) + amt)
				/ (double) (getXpNeededToLevelUp(newLvl));
	...
	public int getCurrentExp() {
		...
		int cur = getXpForLevel(lvl)
				+ (int) Math.round(getXpNeededToLevelUp(lvl) * player.getExp());
		return cur;
	}
	...
	private double getCurrentFractionalXP() {
		...
		int lvl = player.getLevel();
		double cur = getXpForLevel(lvl)
				+ (double) (getXpNeededToLevelUp(lvl) * player.getExp());
		return cur;
	}
	...
	public int getXpForLevel(int level) {
		Validate.isTrue(level >= 0 && level <= hardMaxLevel, "Invalid level "
				+ level + "(must be in range 0.." + hardMaxLevel + ")");
