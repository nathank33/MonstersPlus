...
public class CustomCauldron 
{
	...
	public ItemStack generatePotion()
	{
		...
		//which type of potion it should generate. Then it is added into the effects list
		...
		for(int i = 0; i < cloneMats.size(); i++)
		{
			...
			if(tempMats.size() == 3)
			{
				//If the 3 temp materials are a valid potion, then create the potion effect
		...
		if(mats.contains(BrewList.getIngredient("Splash").getMat())) splash = true;
		if(effects.size() > 0)
		{
	...
	public PotionEffect addCustomEffect(List<Material> items)
	{
		int amplify = 0;
		int extend = 0;
		if(items.contains(BrewList.getIngredient("Amplify 2").getMat())) amplify = 2;
		else if(items.contains(BrewList.getIngredient("Amplify 1").getMat())) amplify = 1;
		if(items.contains(BrewList.getIngredient("Extend 2").getMat())) extend = 2;
		else if(items.contains(BrewList.getIngredient("Extend 1").getMat())) extend = 1;
		...
		if(validEffects.size() > 0) 
		{
			...
			else petype = validEffects.get(0);
				
			if(petype != null)
			{
				BrewIngredient bi = BrewList.getIngredient(petype);	
				PotionEffect effect = new PotionEffect(bi.getEffect(),
						bi.getDurations()[extend] * 20,bi.getAmps()[amplify]);
				return effect;
			}
		}
		return null;
