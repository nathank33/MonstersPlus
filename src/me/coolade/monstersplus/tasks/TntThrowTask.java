...
{	
	...
	public TntThrowTask(LivingEntity lent, int range, int cancelDistance, int maxThrows, int blastTicks, long delay, long repeatDelay)
	{
		this.lent = lent;
		this.range = range;
		...
		else 
		{
			...
			task.setTaskId(Bukkit.getScheduler().scheduleSyncRepeatingTask(MonstersPlus.plugin,task, delay,repeatDelay));
		}
				...
				else
				{
					...
					if(!isValid || lent.isDead() || distance > cancelDistance)
					{
						...
						return;
							...
							&& MonstersPlus.isSpawnableLocation(monsterLoc) && MonstersPlus.isSpawnableLocation(targetLoc))
					{
						/*
						 * Keep track of the maximum amount of throws allowed by the task
						 * In this case we will need to access the concurrentHashMap to grab
						 * the current throws for this specific taskId.
						 */
	...
	public void throwTntAtTarget(LivingEntity lent, LivingEntity target)
	{
		try{
			...
			{
				TNTPrimed tnt = (TNTPrimed) lentLoc.getWorld().spawn(lentLoc, TNTPrimed.class);
