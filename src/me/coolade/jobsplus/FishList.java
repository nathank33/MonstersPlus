...
public class FishList 
{
	...
	{
		/*
		 * This method will go through a cloned/shuffed list of fishable items and try to select one.
		 * It will base the percentage off the fisher job level as a ratio.
		 */
		final double BUFFER = 0.3; //Add a small buffer to allow for a realistic chance at the max level
		...
		{
			double chanceForItem = (((fis.getFinalRate() - fis.getStartRate()) / (double)MAX_LEVEL) * (double)currentLevel)
					+ fis.getStartRate();
			double maxQty = (((fis.getFinalQty() - fis.getStartQty()) / MAX_LEVEL) * currentLevel)
					+ fis.getStartQty() + BUFFER;
	...
	{
		/* The startRate is for a Fisher of level 1, finalRate is for a max level fisher
		 * The startQty is the max amount of drops for a new fisher
