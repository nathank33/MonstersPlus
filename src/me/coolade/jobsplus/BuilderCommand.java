...
public class BuilderCommand 
{
	...
	/*public static final int BIOME_LEVEL = 12;
	public static final int BIOME_COST = 100;
	public static final Biome[] BIOME_TYPES = new Biome[]{Biome.BIRCH_FOREST, Biome.DESERT, Biome.EXTREME_HILLS, Biome.FOREST,
		Biome.ICE_PLAINS, Biome.JUNGLE, Biome.MESA, Biome.PLAINS, Biome.SAVANNA, Biome.SWAMPLAND, Biome.TAIGA};*/
	...
	public BuilderCommand(CommandSender sender, Command cmd, String commandLabel, String[] args)
	{
		...
		{
			/**
			 * A player has the ability to toggle on "Reach" which allows them to place
			 * blocks at a much greater distance away. The event is handled inside of
			 * JobsListener.java.
			 */
		...
		{
			if(level < BIOME_LEVEL){
				player.sendMessage(ChatColor.RED + "You must be a builder level " + ChatColor.GOLD + BIOME_LEVEL + 
						ChatColor.RED + " to use this command.");
				return;
			}
			else if(!MonstersPlus.economy.has(player, BIOME_COST))
			{
	...
	public void displayUsage(Player player)
	{
		...
		player.sendMessage(ChatColor.AQUA + "/builder reach" + ChatColor.WHITE + ": Allows you to place blocks from farther away, type it again to toggle it off.");
		String s = "";
		for(int i = 0; i < REACH_LEVELS.length; i++)
			s = s + "Level " + ChatColor.GOLD + REACH_LEVELS[i] + ChatColor.WHITE + " - " + ChatColor.RED + REACH_DISTANCE[i] +
