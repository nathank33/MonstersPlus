...
public class CustomEnchantListener implements Listener
{
	...
	private static final Material[] SMASH_MATS = new Material[]{Material.STONE,Material.COBBLESTONE,Material.GRAVEL,Material.CLAY,Material.NETHERRACK}; 
	...
	{ 
		...
		{
			entDamager = ((Arrow)entDamager).getShooter();
			wasShot = true;
		}
		
		if(!(entDamager instanceof LivingEntity) || !(entTarget instanceof LivingEntity)) return;
		
		LivingEntity lentTar = (LivingEntity) entTarget;
		LivingEntity lentDam = (LivingEntity) entDamager;
		
		if(!MonstersPlus.isPVPLocation(lentDam.getLocation())) return;
		...
		if(custEnch.hasCustomEnchantmentType(inhandEnchs, "Confusing Aspect"))
		{
			...
			int rank = esoi.getRank();
			lentTar.addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION,(int)(CONFUSING_ASPECT_PR * rank * 20),5));
		}
		...
		if(event.getCause() != DamageCause.CUSTOM && event.getCause() != DamageCause.PROJECTILE)
		{
			...
			if(custEnch.hasCustomEnchantmentType(tarArmorEnchs, "Poison Shell"))
			{
				...
				if(Tools.randomChance(rank * POISON_SHELL_CHANCE_PR))
					lentDam.addPotionEffect(new PotionEffect(PotionEffectType.POISON,(int) (POISON_SHELL_DUR * 20),POISON_SHELL_STR));
			}
			if(custEnch.hasCustomEnchantmentType(tarArmorEnchs, "Wither Shell"))
			{
				...
				if(Tools.randomChance(rank * WITHER_SHELL_CHANCE_PR))
					lentDam.addPotionEffect(new PotionEffect(PotionEffectType.WITHER,(int) (WITHER_SHELL_DUR * 20),WITHER_SHELL_STR));
			}
			if(custEnch.hasCustomEnchantmentType(tarArmorEnchs, "Poverty Barrier"))
			{
				...
				if(Tools.randomChance(rank * POVERTY_BARRIER_CHANCE_PR))
					lentDam.addPotionEffect(new PotionEffect(PotionEffectType.HUNGER,(int) (POVERTY_BARRIER_DUR * 20),POVERTY_BARRIER_STR));
			}
			if(custEnch.hasCustomEnchantmentType(tarArmorEnchs, "Confusing Barrier"))
			{
				...
				if(Tools.randomChance(rank * CONFUSING_BARRIER_CHANCE_PR))
					lentDam.addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION,(int) (CONFUSING_BARRIER_DUR * 20),CONFUSING_BARRIER_STR));
			}
			if(custEnch.hasCustomEnchantmentType(tarArmorEnchs, "Blinding Barrier"))
			{
				...
				if(Tools.randomChance(rank * BLINDING_BARRIER_CHANCE_PR))
					lentDam.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS,(int) (BLINDING_BARRIER_DUR * 20),BLINDING_BARRIER_STR));
			}
			if(custEnch.hasCustomEnchantmentType(tarArmorEnchs, "Regeneration Aura"))
			{
				...
				if(Tools.randomChance(rank * REGEN_AURA_CHANCE_PR))
					lentTar.addPotionEffect(new PotionEffect(PotionEffectType.REGENERATION,(int) (REGEN_AURA_DUR * 20),REGEN_AURA_STR));
			}
			if(custEnch.hasCustomEnchantmentType(tarArmorEnchs, "Health Aura"))
			{
				...
				if(Tools.randomChance(rank * HEALTH_AURA_CHANCE_PR))
					lentTar.addPotionEffect(new PotionEffect(PotionEffectType.ABSORPTION,(int) (HEALTH_AURA_DUR * 20),HEALTH_AURA_STR));
			}
			if(custEnch.hasCustomEnchantmentType(tarArmorEnchs, "Berserk Aura"))
			{
				...
				if(Tools.randomChance(rank * BERSERK_AURA_CHANCE_PR))
					lentTar.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE,(int) (BERSERK_AURA_DUR * 20),BERSERK_AURA_STR));
			}
		}
	...
	{ 
		...
		else if(eventState == State.CAUGHT_ENTITY && ent instanceof LivingEntity)
		{
			...
			if(!MonstersPlus.isPVPLocation(lentTar.getLocation())) return;
	...
	{ 
		...
		{
			if(!MonstersPlus.isBuildLocation(player, loc)) return;
		...
		if(custEnch.hasCustomEnchantmentType(inhandEnchs, "Smash"))
		{
			...
			if((block.getType() == Material.STONE || block.getType() == Material.GRAVEL) && Tools.randomChance(SMASH_PR * rank))
			{
				...
				for(int z = SMASH_RADIUS * -1; z <= SMASH_RADIUS; z++)
					{
						...
						if(Arrays.asList(SMASH_MATS).contains(tblock.getType()))
							if(MonstersPlus.isBuildLocation(player, tblock.getLocation()) && !(x == 0 && y == 0 && z == 0))
							{
								if(tblock.getType() == Material.STONE)
								{
									if(inhand.getEnchantments().containsKey(Enchantment.SILK_TOUCH))
										world.dropItemNaturally(tblock.getLocation(), new ItemStack(Material.STONE));
									else
										world.dropItemNaturally(tblock.getLocation(), new ItemStack(Material.COBBLESTONE));
								...
								else if(tblock.getType() == Material.GRAVEL)
								{
									...
									if(Tools.randomChance(25))
										world.dropItemNaturally(tblock.getLocation(), new ItemStack(Material.FLINT));
									else
										world.dropItemNaturally(tblock.getLocation(), new ItemStack(Material.GRAVEL));
								}
								else
								{
	...
	public void onPlayerInteract(PlayerInteractEvent event) 
	{
		...
		if(event.hasBlock())
		{
			...
			List<EnchantmentSetOnItem> inhandEnchs = custEnch.getCustomEnchantments(inhand);
			if(custEnch.hasCustomEnchantmentType(inhandEnchs, "Tilling") && event.getAction() == Action.RIGHT_CLICK_BLOCK)
			{
				...
				for(int x = radius * -1; x < radius; x++)
					for(int z = radius * -1; z < radius; z++)
					{
						...
						if(tblock.getType() == Material.DIRT || tblock.getType() == Material.GRASS)
							if(tblock.getType() != Material.SOIL && MonstersPlus.isBuildLocation(player, tblock.getLocation()) 
							&& !(x == 0 && z == 0))
							{
			...
			if(custEnch.hasCustomEnchantmentType(inhandEnchs, "Seed Finder") && event.getAction() == Action.RIGHT_CLICK_BLOCK)
			{
			...
			if(custEnch.hasCustomEnchantmentType(inhandEnchs, "Hydration") && event.getAction() == Action.RIGHT_CLICK_BLOCK)
			{
			...
			if(custEnch.hasCustomEnchantmentType(inhandEnchs, "Papercut") && event.getAction() == Action.RIGHT_CLICK_BLOCK)
			{
				...
				else if(block.getType() == Material.DEAD_BUSH && MonstersPlus.isBuildLocation(player, block.getLocation()) )
				{
					block.setType(Material.AIR);
	...
	{
		/**
		 * Due to the bug caused by enderchests, in which custom enchantments were being renamed
		 * with a weird purple A, we will use this method to check and clean up any bugged enchantment names.
		 */
		if(event.isCancelled()) return;
		...
		if(item.hasItemMeta() && item.getItemMeta().hasLore())
		{
			...
			List<EnchantmentSet> esets = custEnch.getEnchantmentList();
