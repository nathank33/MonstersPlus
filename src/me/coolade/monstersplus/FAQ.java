...
public class FAQ 
{
	...
	public static String[] topics = {"Money","Market","Monsters","Quests","Jobs", "Survivalist", "Miner", "Warrior",
		"Fisherman", "WitchDoctor", "Farmer", "Enchanter", "Builder", "Blacksmith", "Disguise",
		"Customarrows", "Oreradar", "Tracking", "Brewing", "Fishing", "Jobenchant", "Customenchant", 
		"ArrowRecipes", "Farmer Recipes","Builder Recipes","Trophy", "Powerstones", "ExpTrading", 
		...
		else if(args.length > 0)
		{
			if(args[0].equalsIgnoreCase("Jobs") && args.length == 1){
				sender.sendMessage(ChatColor.GOLD + "Jobs: " + ChatColor.WHITE + 
						...
						"To learn more about jobs type: " +
						ChatColor.RED + "/jobs, /jobs help, /jobs browse, /jobs join, /faq jobs explist" +
			...
			else if(args[0].equalsIgnoreCase("jobs") && args.length > 1 &&
					args[1].equalsIgnoreCase("explist")){
				...
				sender.sendMessage(ChatColor.GOLD + "Survivalist: " + ChatColor.WHITE + 
						...
						"building wooden structures, killing animals, and killing certain monsters. " +
						ChatColor.AQUA + "Unique abilities: Custom Arrow Recipes. " + 
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/faq customarrows, /faq arrowrecipes, /jobs info survivalist");
				sender.sendMessage(ChatColor.DARK_RED + "Spawner based monsters are ignored for experience and money. ");
			}
			else if(args[0].equalsIgnoreCase("Miner")){
				sender.sendMessage(ChatColor.GOLD + "Miner: " + ChatColor.WHITE + 
						"The Miner receives money by collecting precious ore such as diamond. " + 
						ChatColor.AQUA + "Unique Abilities: Ore Radar. " +
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/jobs info miner, /faq oreradar");
			}
			else if(args[0].equalsIgnoreCase("Warrior")){
				sender.sendMessage(ChatColor.GOLD + "Warrior: " + ChatColor.WHITE + 
						"The Warrior receives money for all monsters and animals. " + 
						ChatColor.AQUA + "Unique Abilities: Monster Tracking, Custom Arrow Recipes, challenge. " +
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/jobs info warrior, /faq tracking, /faq customarrows, /faq challenge");
				sender.sendMessage(ChatColor.DARK_RED + "Spawner based monsters are ignored for experience and money. ");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Fisherman: " + ChatColor.WHITE + 
						...
						ChatColor.AQUA + "Unique Abilities: Custom Fishing "+
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/jobs info fisherman, /faq fishing");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Witchdoctor: " + ChatColor.WHITE + 
						...
						"potion effects to be added onto a single potion. " +
						ChatColor.AQUA + "Unique Abilities: Custom Brewing "+
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/jobs info witchdoctor, /faq brewing, /faq brewing process, /faq brewing list");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Farmer: " + ChatColor.WHITE + 
						...
						ChatColor.AQUA + "Unique Abilities: Animal Tracking, Custom Farmer Recipes, Spawn Egg Crafting"+
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/jobs info farmer, /faq tracking, /faq farmer recipes");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Enchanter: " + ChatColor.WHITE + 
						...
						ChatColor.AQUA + "Unique Abilities: Direct Enchanting (/jobenchant), All Custom Enchantments (/customenchant)" +
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/jobs info enchanter, /faq jobenchant, /faq customenchant, /faq powerstones");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Builder: " + ChatColor.WHITE + 
						...
						"Builders can use commands to place blocks from farther away (/builder)." + 
						ChatColor.AQUA + "Unique Abilities: Custom Block Recipes, Mob Spawner Changing, EnderDragon Spawning " +
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/jobs info builder, /faq builder recipes, /faq powerstones, /faq EnderDragonSpawning, /builder");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Blacksmith: " + ChatColor.WHITE + 
						...
						ChatColor.AQUA + "Unique Abilities: " +
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/jobs info blacksmith");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Disguise: " + ChatColor.WHITE + 
						...
						ChatColor.WHITE + "To learn more type: " + 
						ChatColor.RED + "/disguise, /disguise display, /disguise display available");				
			}
				...
				{
					sender.sendMessage(ChatColor.AQUA + rs.getName() + " - " + ChatColor.WHITE + 
							rs.getDescription() + " (" + rs.getJob() + " level " + rs.getLevel() + ")");
				}
				sender.sendMessage(ChatColor.WHITE + "To find the recipes type: " +
						ChatColor.RED + "/faq arrowrecipes");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Ore Radar: " + ChatColor.WHITE + 
						...
						OreRadar.COOLDOWN_MINUTE + " minutes. " +
						ChatColor.GOLD + "To display the ore levels type: " + 
						ChatColor.RED + "/faq oreradar levels");
			}
			else if(args[0].equalsIgnoreCase("OreRadar") && args.length > 1 &&
					args[1].equalsIgnoreCase("levels")){
				sender.sendMessage(ChatColor.GOLD + "Ore Radar Levels: ");
				for(OreSet os : OreRadar.oreSets)
				{
					sender.sendMessage(ChatColor.AQUA + os.getMat().toString() + ChatColor.WHITE + ": Miner level " + os.getLevel() + 
							" Scan Radius: " + os.getRadius());
				}
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Tracking: " + ChatColor.WHITE + 
						...
						"will turn the user to the monster or animal. " +
						"The tracker scans the nearby area once " +
						"every 30 seconds, continues for " + Tracker.MINUTE + " minutes, and has a cooldown of " +
						Tracker.COOLDOWN_MINUTE + " minutes." +
						ChatColor.GOLD + "To display the tracking levels type: " + 
						ChatColor.RED + "/faq tracking warrior, /faq tracking farmer");
			}
			else if(args[0].equalsIgnoreCase("tracking") && args.length > 1 &&
					args[1].equalsIgnoreCase("warrior")){
				sender.sendMessage(ChatColor.GOLD + "Tracking Warrior: ");
				for(TrackSet ts : Tracker.trackSets)
				{
					if(ts.getJob().equalsIgnoreCase("Warrior")){
					sender.sendMessage(ChatColor.AQUA + ts.getType().toString() + ChatColor.WHITE + ": Warrior level " + ts.getLevel() + 
							" Scan Radius: " + ts.getRadius());
					}
				}
			}
			else if(args[0].equalsIgnoreCase("tracking") && args.length > 1 &&
					args[1].equalsIgnoreCase("farmer")){
				sender.sendMessage(ChatColor.GOLD + "Tracking Warrior: ");
				for(TrackSet ts : Tracker.trackSets)
				{
				...
				sender.sendMessage(ChatColor.GOLD + "Brewing: " + ChatColor.WHITE + 
						...
						BrewingListener.INGREDIENT_EXP + " per ingredient added into the cauldron. " +
						ChatColor.GOLD + "To display the Brewing process and ingredient list type: " + 
						ChatColor.RED + "/faq brewing process, /faq brewing list");
			}
			else if(args[0].equalsIgnoreCase("brewing") && args.length > 1 &&
				...
				ChatColor.GOLD + "To display the Fishing Tiers and items type: " + 
				ChatColor.RED + "/faq fishing list");
			}
			else if(args[0].equalsIgnoreCase("fishing") && args.length > 1 &&
					args[1].equalsIgnoreCase("list")){
				...
				"get access to more enchantments. " + 
				ChatColor.GOLD + "To display the Jobenchant list type: " + 
				...
				for(int i = (page-1) * amtPerPage; i < EnchantList.ENCHANTLIST.size(); i++)
				{
					...
					sender.sendMessage(ChatColor.AQUA + er.getName() + ChatColor.WHITE + " " +
							er.getLevel() + ", " + er.getJob() + " Level: " + er.getJobLevel() + " ExpCost: " + er.getExpCost());
				}
				...
				sender.sendMessage(ChatColor.GOLD + "EnderDragon Spawning: " + ChatColor.WHITE + 
						"Level " + ChatColor.RED + JobsListener.ENDERSPAWNING_LEVEL + " builders " + ChatColor.WHITE +
				...
				{
					sender.sendMessage(ChatColor.AQUA + rs.getName() + " - " + ChatColor.WHITE + 
							" (" + rs.getJob() + " level " + rs.getLevel() + ") Powerstones: " + rs.getPowerstones());
				...
				{
					sender.sendMessage(ChatColor.AQUA + rs.getName() + " - " + ChatColor.WHITE + 
							" (" + rs.getJob() + " level " + rs.getLevel() + ") " + rs.getDescription());
					sender.sendMessage(ChatColor.WHITE + "   " + rs.getRecipeDescription());
				}
			}
			else if(args[0].toLowerCase().contains("monster") && args.length == 1){
				sender.sendMessage(ChatColor.GOLD + "Monsters: " + ChatColor.WHITE + 
						"The monsters in this server are unique and come in 5 difficulties: " +
						"Normal, " + ChatColor.YELLOW + "Easy, " + ChatColor.GOLD + "Medium, " +
						ChatColor.RED + "Hard, " + ChatColor.DARK_PURPLE + "Legendary. " +
						ChatColor.WHITE + "Legendary monsters are rare andextremely dangerous, but " +
					...
					ChatColor.GOLD + "To display more information type: " + 
					ChatColor.RED + "/faq powerstones, /faq trophy");
			}
				...
				sender.sendMessage(ChatColor.GOLD + "Monsters: " + ChatColor.WHITE + 
						...
						ChatColor.WHITE + " trophies in existance. " +
					ChatColor.GOLD + "To display more information type: " + 
					ChatColor.RED + "/faq monsters");
			}
			else if(args[0].equalsIgnoreCase("exptrading") && args.length == 1){
				sender.sendMessage(ChatColor.GOLD + "Exp Trading: " + ChatColor.WHITE + "Right click an enchantment table with a Bucket " +
				...
				sender.sendMessage(ChatColor.GOLD + "Custom Enchant: " + ChatColor.WHITE + 
						...
						"However, Enchanters have access to every custom enchantment. " +
						ChatColor.GOLD + "To display more information type: " + 
						ChatColor.RED + "/customenchant, /faq powerstones");
			}
				...
				sender.sendMessage("Powerstones can be used to give your item a random enchantment. " + ChatColor.GREEN + " (/powerstone enchant) " + ChatColor.WHITE);
				sender.sendMessage("Powerstones can convert item enchantments into book enchantments. " + ChatColor.GREEN + "(/powerstone unenchant) " + ChatColor.WHITE);
				sender.sendMessage("Powerstones can be converted into Job Experience. " + ChatColor.GREEN + " (/powerstone exp <jobname>) " + ChatColor.WHITE);
				sender.sendMessage("Powerstones can create custom enchantments such as Lifesteal. " + ChatColor.GREEN + " (/Customenchant) " + ChatColor.WHITE);
				sender.sendMessage("Powerstones can be used by builders to change Mobspawner types." + ChatColor.GREEN + " (/faq builder recipes) " + ChatColor.WHITE); 
				sender.sendMessage("Powerstones can be used by Blacksmiths to repair items." + ChatColor.GREEN + " (/powerstone, /powerstone repair, /powerstone repairall) " + ChatColor.WHITE); 		
				sender.sendMessage(ChatColor.WHITE + "Unfortunately, powerstones are rare and are most often found by the strongest monsters. " + 
						"Drop Chances: " + "Normal - " + MonsterList.PSTONE_NORMAL + "% " +
						ChatColor.YELLOW + "Easy - " + MonsterList.PSTONE_EASY + "% " +
						ChatColor.GOLD + "Medium - " + MonsterList.PSTONE_MEDIUM + "% " +
						ChatColor.RED + "Hard - " + MonsterList.PSTONE_HARD + "% " +
						ChatColor.DARK_PURPLE + "Legendary - " + MonsterList.PSTONE_LEGENDARY + "% " +
						ChatColor.GOLD + "To display more information type: " + 
						ChatColor.RED + "/powerstone, /faq monsters, /faq trophy, /faq customenchant, /faq builder");
			}
			...
			else if(args[0].equalsIgnoreCase("market") && args.length == 1)
			{
				sender.sendMessage(ChatColor.GOLD + "Market: " + ChatColor.WHITE + 
						...
						"To see available items type /market listings. " +
						"To sell an item do /market create. " + 
						"To collect an item or money do /market mail. " +
						ChatColor.GOLD + "To display more information type: " + 
						ChatColor.RED + "/market help");
			}
			else if(args[0].equalsIgnoreCase("quests") && args.length == 1)
			{
				sender.sendMessage(ChatColor.GOLD + "Quests: " + ChatColor.WHITE + 
						"Quests are offered by NPC's around the spawn. " +
