...
import me.coolade.monstersplus.tasks.RepeatableLivingEntityTask;
...
public class MonsterUpdater 
{
	...
	public MonsterUpdater(EntityDeathEvent event)
	{
		...
		final int rad = 30; //If there are players within this range then continue
	...
	public MonsterUpdater(LivingEntity damager, LivingEntity target, boolean isProjectile)
	{
		...
		else if((MonsterList.isCustomMonster("Starving Creeper",target) || MonsterList.isCustomMonster("Famished Lurker",target)) && (damager instanceof Player))
		...
		//Fire Effects
		else if((MonsterList.isCustomMonster("Burning Walker",target) || MonsterList.isCustomMonster("Burnt Ghoul",target) 
				||  MonsterList.isCustomMonster("Hot Bones",target) || MonsterList.isCustomMonster("Desert Scorpion",target))
				&& (damager instanceof Player))
			Tools.burnEntity(damager, 2 * 20, 40);
		else if(MonsterList.isCustomMonster("Burning Walker",damager) || MonsterList.isCustomMonster("Burnt Ghoul",target))
		...
		//Ice effects
		else if((MonsterList.isCustomMonster("Cold Corpse",target) || MonsterList.isCustomMonster("Frosted Biter",target)) && (damager instanceof Player))
			Tools.potionEffectEntity(damager, PotionEffectType.SLOW, 1, 60, 50);
		else if(MonsterList.isCustomMonster("Arctic Zed",damager) || MonsterList.isCustomMonster("Iced Sharpshooter",damager) 
				|| MonsterList.isCustomMonster("Spinechiller",damager) || MonsterList.isCustomMonster("Archfiend",damager)
	...
	public MonsterUpdater(ProjectileLaunchEvent event)
	{
		...
		{
			LivingEntity lent = ((Arrow)event.getEntity()).getShooter();
			if(lent == null) return;
	...
	{
		if(!(lent instanceof Monster)) return;
	...
	{	
		//Set the maxhealth lower so that that we know that this monster has been updated
	...
	public void updateEnteiAttack(LivingEntity damager, LivingEntity target)
	{
		...
		if(Tools.randomChance(40))
		{
			...
			if(MonstersPlus.isSpawnableLocation(destBlock.getLocation()) && destBlock.getType().equals(Material.AIR))
				if(target instanceof Player && MonstersPlus.isBuildLocation((Player)target, destBlock.getLocation()))
	...
	public static void createIceRing(LivingEntity target, double seconds, boolean useSlow)
	{
		...
		for(Block block : blocks)
		{
			if(MonstersPlus.isSpawnableLocation(block.getLocation()) && block.getType().equals(Material.AIR))
				//if(MonstersPlus.isBuildLocation((Player)target, block.getLocation()))
				{
					//Make sure to try and remove water aswell incase some of the ice melts.
					...
					new CleanBlockTask(Material.STATIONARY_WATER,Material.AIR, block.getLocation(),(int)(seconds * 20)*2);
					if(useSlow)
		...
		{			
					...
					&& MonstersPlus.isBuildLocation((Player)damager, damager.getLocation()))
			{
				for(int i = 0; i < 5; i++)
				{
					...
					Vector vec = damager.getLocation().toVector().subtract(target.getLocation().add(x,i * YDIFF,z).toVector());
					Tools.shootWithAngles(target,target.getLocation().add(x,i * YDIFF,z),vec,1,angles,"largefireball");
				}
			}
		}
	}
	...
	public void updateWilburDamaged(LivingEntity damager, LivingEntity target)
	{
		...
		if(Tools.randomChance(15))
		{
			for(int i = 0; i < total; i++)
			{
				...
				{
					LivingEntity pzomb = (LivingEntity)blockLoc.getWorld().spawnEntity(blockLoc, EntityType.PIG_ZOMBIE);
	...
	public static void potionEffectEntityAOE(Location loc, int radius, PotionEffectType type, int strength, int duration, double chance)
	{
		if(Tools.randomChance(chance))
		{
