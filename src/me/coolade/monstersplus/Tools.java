...
public class Tools 
{
	...
	{
		//Iterates through a radius of blocks and finds if any are matching the parameter material
	...
	{
		/*
		 * This method organizes a list of entities that surround a certain location.
		 * They are organized by closest to furthest.
		 */
		...
		while(keepGoing == true)
		{
			...
			{
				if(getDistanceSquared(loc,entList.get(i).getLocation()) > getDistanceSquared(loc,entList.get(i+1).getLocation()))
				{
					keepGoing = true;
					Entity temp;
					temp = entList.get(i);
	...
	public static boolean hasEmptyFaces(Location loc)
	{
		...
		if(world.getBlockAt((int)loc.getX() + 1,(int)loc.getY(),(int)loc.getZ()).getType() == Material.AIR)
			if(world.getBlockAt((int)loc.getX() - 1,(int)loc.getY(),(int)loc.getZ()).getType() == Material.AIR)
				if(world.getBlockAt((int)loc.getX(),(int)loc.getY(),(int)loc.getZ() + 1).getType() == Material.AIR)
					if(world.getBlockAt((int)loc.getX() + 1,(int)loc.getY(),(int)loc.getZ() - 1).getType() == Material.AIR)
						return true;
		return false;
	}
	public static ItemFrame addItemFrame(Block block, BlockFace face)
	{
		...
		if(northBlock.getType() == Material.AIR) northBlock.setType(mat);
		if(southBlock.getType() == Material.AIR) southBlock.setType(mat);
		if(eastBlock.getType() == Material.AIR) eastBlock.setType(mat);
		if(westBlock.getType() == Material.AIR) westBlock.setType(mat);
		
		if(face == BlockFace.NORTH) northBlock.setType(Material.AIR);
		else if(face == BlockFace.SOUTH) southBlock.setType(Material.AIR);
		else if(face == BlockFace.EAST) eastBlock.setType(Material.AIR);
		else if(face == BlockFace.WEST) westBlock.setType(Material.AIR);
		
		ItemFrame itemFrame = (ItemFrame)block.getWorld().spawnEntity(block.getLocation(), EntityType.ITEM_FRAME);
		
		if(northBlock.getType() == mat) northBlock.setType(Material.AIR);
		if(southBlock.getType() == mat) southBlock.setType(Material.AIR);
		if(eastBlock.getType() == mat) eastBlock.setType(Material.AIR);
		if(westBlock.getType() == mat) westBlock.setType(Material.AIR);
	...
	public static List<String> getNearbyPlayers(Location loc,double radius)
	{
		...
		Player[] nbPlayers = Bukkit.getOnlinePlayers();
		for(Player player : nbPlayers)
		{
	...
	public static ItemFrame getFaceItemFrame(Location loc, BlockFace face)
	{
		...
		{
			if((int)ent.getLocation().getYaw() == 0 && face == BlockFace.NORTH) return (ItemFrame) ent;
			else if((int)ent.getLocation().getYaw() == 90 && face == BlockFace.WEST) return (ItemFrame) ent;
			else if((int)ent.getLocation().getYaw() == 180 && face == BlockFace.SOUTH) return (ItemFrame) ent;
			else if((int)ent.getLocation().getYaw() == 270 && face == BlockFace.EAST) return (ItemFrame) ent;
		}
		return null;
	}
	...
	public static void removeNearbyEntityDelay(final Location loc, final Material mat, final double radius, final long delay)
	{
	...
	public static boolean isPowerstone(ItemStack item)
	{
		if(item == null || !item.hasItemMeta() || !item.getItemMeta().hasDisplayName() || 
				!item.getItemMeta().hasLore()) return false;
    ...
    public static ItemStack silkTouchItem(ItemStack item)
    {
    	...
    	if(mat.equals(Material.COAL)) item.setType(Material.COAL_ORE);
    	else if(mat.equals(Material.DIAMOND)) item.setType(Material.DIAMOND_ORE);
    	else if(mat.equals(Material.EMERALD)) item.setType(Material.EMERALD_ORE);
    	else if(mat.equals(Material.QUARTZ)) item.setType(Material.QUARTZ_ORE);
    	else if(mat.equals(Material.DIRT)) item.setType(Material.GRASS);
    	else if(mat.equals(Material.COBBLESTONE)) item.setType(Material.STONE);
    ...
    {
    	Material mat = item.getType();
    	if(mat.equals(Material.RAW_FISH)) item.setType(Material.COOKED_FISH);
    	else if(mat.equals(Material.RAW_BEEF)) item.setType(Material.COOKED_BEEF);
    	else if(mat.equals(Material.RAW_CHICKEN)) item.setType(Material.COOKED_CHICKEN);
    	else if(mat.equals(Material.PORK)) item.setType(Material.GRILLED_PORK);
    	else if(mat.equals(Material.IRON_ORE)) item.setType(Material.IRON_INGOT);
    	else if(mat.equals(Material.GOLD_INGOT)) item.setType(Material.GOLD_INGOT);
    	return item;
    }
    ...
    {
    	/*
    	 * Adds up the amount of specific types of enchantments and checks if the total is greater than 1.
    	 * If it is greater than 1 than we know that there are multiple enchants that would conflict.
    	 */
    	...
    	int damagers = 0;
    	int pickaxers = 0;
    	if(enchs.containsKey(Enchantment.PROTECTION_ENVIRONMENTAL)) protectors++;
    	if(enchs.containsKey(Enchantment.PROTECTION_EXPLOSIONS)) protectors++;
    	if(enchs.containsKey(Enchantment.PROTECTION_FIRE)) protectors++;
    	if(enchs.containsKey(Enchantment.PROTECTION_PROJECTILE)) protectors++;
    	if(enchs.containsKey(Enchantment.DAMAGE_ALL)) damagers++;
    	if(enchs.containsKey(Enchantment.DAMAGE_ARTHROPODS)) damagers++;
    	if(enchs.containsKey(Enchantment.DAMAGE_UNDEAD)) damagers++;
    	if(enchs.containsKey(Enchantment.SILK_TOUCH)) pickaxers++;
    	if(enchs.containsKey(Enchantment.LOOT_BONUS_BLOCKS)) pickaxers++;
    ...
    public static boolean inFactionTerritory(String playerName)
    {
    	...
    	Faction areaFaction = BoardColls.get().getFactionAt(PS.valueOf(player.getLocation()));
    	UPlayer uplayer = UPlayer.get(player);
    	Faction playerFaction = uplayer.getFaction();
    	
    	Bukkit.broadcastMessage("PFac:" + playerFaction.getName() + "  AFac:" + areaFaction.getName());
	...
	public static void shootWithAngles(LivingEntity player, Location location, Vector direction, double speed, int[] angles, String projType) 
	{
		...
		for (int angle : angles) 
		{
			...
			if(projType.equalsIgnoreCase("Snowball"))
				proj = (Projectile) location.getWorld().spawn(location, Snowball.class);
			else if(projType.equalsIgnoreCase("Fireball"))
				proj = (Projectile) location.getWorld().spawn(location, Fireball.class);
			else if(projType.equalsIgnoreCase("Witherskull"))
				proj = (Projectile) location.getWorld().spawn(location, WitherSkull.class);
			else if(projType.equalsIgnoreCase("SmallFireball"))
				proj = (Projectile) location.getWorld().spawn(location, SmallFireball.class);
			else if(projType.equalsIgnoreCase("LargeFireball"))
				proj = (Projectile) location.getWorld().spawn(location, LargeFireball.class);
			else
				proj = (Projectile) location.getWorld().spawn(location, Arrow.class);
	...
	public static void damageEntity(LivingEntity damager, LivingEntity target, double damage)
	{
		...
		target.setLastDamageCause(new EntityDamageByEntityEvent(damager,
				target, DamageCause.CUSTOM, damage));
	}
	...
	public static void dropRandomSeed(Location loc)
	{
		...
		Material[] mats = new Material[]{Material.SEEDS,Material.MELON_SEEDS,Material.CARROT,Material.POTATO,Material.PUMPKIN_SEEDS};
		ItemStack istack = new ItemStack(mats[random.nextInt(mats.length)]);
		loc.getWorld().dropItemNaturally(loc, istack);
	
	}
	public static void potionEffectEntity(LivingEntity lent, PotionEffectType type, int strength, int duration, double chance)
	{
