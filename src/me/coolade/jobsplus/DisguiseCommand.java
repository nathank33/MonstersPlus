...
public class DisguiseCommand
{
	...
	{
		/**
		 * Sets the player to a specific disguise, and displays information about it.
		 * NOTE: The args is setup different in this compared to the normal commands, because these
		 * arguments are based on split.
		 */
		...
		else if(args[1].equalsIgnoreCase("display"))
		{
			...
			else if(args[2].equalsIgnoreCase("available"))
			{
				...
				for(DisguiseSet ds : disguiseList)
					if(JobsListener.getJobLevel(player.getName(), ds.getJob()) >= ds.getLevel() || player.isOp() || player.hasPermission("monstersplus.disguises"))
						displayDisguise(player,ds);
			}
			else
			{
		...
		else
		{
			...
			if(JobsListener.getJobLevel(player.getName(), dset.getJob()) < dset.getLevel()  && !player.isOp() && !player.hasPermission("monstersplus.disguises")){
				player.sendMessage(ChatColor.RED + "You must be a level " + ChatColor.GOLD + dset.getLevel() + " " + dset.getJob() + ChatColor.RED + " to be disguised as " + ChatColor.GOLD + dset.getName());
				return;
			}
			
			MobDisguise disg = null;
			String name = dset.getName();	
			if(name.equalsIgnoreCase("SkeletonHorse")) 		disg = new MobDisguise(DisguiseType.SKELETON_HORSE,true);
			else if(name.equalsIgnoreCase("Horse")) 		disg = new MobDisguise(DisguiseType.HORSE,true);
			else if(name.equalsIgnoreCase("Mule"))			disg = new MobDisguise(DisguiseType.MULE,true);
			else if(name.equalsIgnoreCase("Donkey"))		disg = new MobDisguise(DisguiseType.DONKEY,true);
			else if(name.equalsIgnoreCase("Ocelot"))		disg = new MobDisguise(DisguiseType.OCELOT,true);
			else if(name.equalsIgnoreCase("IronGolem"))		disg = new MobDisguise(DisguiseType.IRON_GOLEM,true);
			else if(name.equalsIgnoreCase("MagmaCube"))		disg = new MobDisguise(DisguiseType.MAGMA_CUBE,true);
			else if(name.equalsIgnoreCase("ZombieHorse"))	disg = new MobDisguise(DisguiseType.UNDEAD_HORSE,true);
			else if(name.equalsIgnoreCase("WitherSkeleton"))disg = new MobDisguise(DisguiseType.WITHER_SKELETON,true);
			else if(name.equalsIgnoreCase("Wither"))		disg = new MobDisguise(DisguiseType.WITHER,true);
			else if(name.equalsIgnoreCase("BabyPigman"))	disg = new MobDisguise(DisguiseType.PIG_ZOMBIE,true,true);
			else if(name.equalsIgnoreCase("BabyZombie"))	disg = new MobDisguise(DisguiseType.ZOMBIE,true,true);
			else if(name.equalsIgnoreCase("BabyChicken"))	disg = new MobDisguise(DisguiseType.CHICKEN,true,true);
			else if(name.equalsIgnoreCase("BabyPig"))	disg = new MobDisguise(DisguiseType.PIG,true,true);
			else if(name.equalsIgnoreCase("BabyCow"))	disg = new MobDisguise(DisguiseType.COW,true,true);
			else if(name.equalsIgnoreCase("ZombieVillager"))disg = new MobDisguise(DisguiseType.ZOMBIE_VILLAGER,true);
			...
			else if(name.equalsIgnoreCase("GoldSkeleton"))
			{
				...
				LivingWatcher watcher = (LivingWatcher) disg.getWatcher();
				watcher.setArmor(new ItemStack[]{new ItemStack(Material.GOLD_BOOTS),
			...
			if(disg != null)
			{
				...
				player.sendMessage(ChatColor.AQUA + "You are now being disguised as: " + ChatColor.GOLD + dset.getName());
	...
	public void displayUsage(Player player)
	{
		...
		player.sendMessage(ChatColor.AQUA + "/disguise display: " + ChatColor.WHITE + "Display the disguises for all jobs.");
		player.sendMessage(ChatColor.AQUA + "/disguise display <job>: " + ChatColor.WHITE + "Display all the available disguises for a specific job.");
		player.sendMessage(ChatColor.AQUA + "/disguise display available: " + ChatColor.WHITE + "Display all the disguises that you can use.");
	...
	public void displayDisguise(Player player, DisguiseSet dset)
	{
		 ...
		 */
		player.sendMessage(ChatColor.AQUA + dset.getName() + " " + ChatColor.WHITE + dset.getJob() + " level " + dset.getLevel());
	}
}
